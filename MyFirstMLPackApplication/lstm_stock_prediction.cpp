                          ...
                          const size_t rho)
{
  ...
  {
    //dataset.n_cols - rho (655) сдвигов (колонок), в каждом из которых rho (25) точек (срезов) по 5 значений (строк)
    X.subcube(arma::span(), arma::span(i), arma::span()) =
        dataset.submat(arma::span(), arma::span(i, i + rho - 1));
    //dataset.n_cols - rho (655) сдвигов, в каждом из которых rho (25) точек по 2 значения (колонки 4 и 5 в исходных данных)
                 ...
                 const arma::cube& testX)
{
  ...
  data::Save(filename, flatDataAndPreds, false, true, data::FileType::CSVASCII);
...
int run_lstm()
{
  ...
  trainX.set_size(inputSize, trainData.n_cols - rho, rho);//5 - 655 - 25
  trainY.set_size(outputSize, trainData.n_cols - rho, rho);//2 - 655 - 25
  testX.set_size(inputSize, testData.n_cols - rho, rho);//5 - 50 - 25
  testY.set_size(outputSize, testData.n_cols - rho, rho);//2 - 50 - 25
  ...
  //passes all row data except date as input
  cout << "Train data:" << endl;
  normal_print(cout, trainX.slice(0), true);
  //passes high and low as rows
  cout << "Train labels:" << endl;
  normal_print(cout, trainY.slice(0), true);
  //trainY.brief_print(cout, "Train labels");
  /*cout << "Test data:" << endl;
  testX.brief_print(cout, "");
  testY.brief_print(cout, "");*/
  ...
  std::array<double, 3> percents = calculateDifferencePrecents(testY, predOutP);
  cout << "Deviation in percents (min, avg, max): " << percents[0] << " " << percents[1] << " " << percents[2] << endl;
